...
TEMPLATE_NPY = 'template_msg.npy'
SYNC_VERTEX_NPY = 'template_sync_v.npy'
SYNC_EDGE_NPY = 'template_sync_e.npy'
...
def main():
    ...
    print("payload device_id: 0x%08x" % int(meta["device_id"]))

    # 推荐打印1：生成时间（最可靠）
    if "generated_utc" in meta:
        print("payload generated_utc:", meta["generated_utc"])
    else:
        print("payload generated_utc: None")

    # 推荐打印2：结构化时间
    dtf = meta.get("datetime_fields")
    if dtf is not None:
        print(
            "payload datetime utc: %04d-%02d-%02d %02d:%02d:%02d UTC"
            % (
                dtf["year"],
                dtf["month"],
                dtf["day"],
        ...
        print("payload datetime utc: None")
    print('payload device_id: 0x%08x' % int(meta.get('device_id', 0)))
    ...
    for (x0, y0) in positions:
        ...
        ok = blend_template(y_channel, tm, x0, y0, ALPHA)
        if not ok:
            raise ValueError('failed to embed message block')

    for (vx, vy) in vertex_points:
        if blend_template_centered(y_channel, sync_vertex, vx, vy, SYNC_ALPHA):
    ...
    for (ex, ey) in edge_points:
        if blend_template_centered(y_channel, sync_edge, ex, ey, SYNC_ALPHA):
            all_sync_boxes.append((
                int(round(ex - ss / 2)),
                int(round(ey - ss / 2)),
                ss, ss, 'E'
